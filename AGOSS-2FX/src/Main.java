...
public class Main {
    ...
    //Present pane that asks for user input
    public static void nameSelection(int type) {
		//Frame.fadeIn();
    	frame.add(Frame.charName(type));
    }
    
    //Checks to see if file is valid
    public static void newGame(String name) throws Exception {

	    File tmpDir = new File("src\\saves\\" + name);
	    boolean exists = tmpDir.exists();
	    if (exists) {
	    	//Present error if file exists
	    	Frame.setlblError();
	    } else {
	    	//If so continue
    ...
    public static void characterCreator(String tempName, int newGame) throws Exception {
	   ...
	   ////////////////////////////
	    ...
	    if(newGame == 1) {
			...
			/////////////////////////////
		    Player player = new Player(tempName, 5, 5, 5, 50, 10, 1, 0, 0);
		    
		    //send player and add pane
		    frame.add(Frame.newCharacter(player));
